...
def merge_movie(movie_list1):
    ...
    subprocess.run(["ffmpeg", "-f", "concat", "-safe", "0", "-i", "tmp.txt",
                   "-c", "copy", FileName], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    subprocess.run(["mkdir", "/var/www/merged"])
    subprocess.run(["mv", "out.mp4", "/var/www/merged"])


if __name__ == "__main__":
    start = time.time()
    subprocess.run(["mkdir", "/var/www/cut"])
    input_movie = get_movie("/var/www/input")

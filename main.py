...
def get_movie(wk_dir: str) -> str:
    '''指定ディレクトリにアップロードされている動画を取得するメソッド'''
    files = os.listdir(wk_dir)
    files = natsort.natsorted(
        [x for x in files if x[-4:] == ".MOV" or x[-4:] == ".mp4" or x[-4:] == ".mov"])
    return files
...
def cut_silent(movie1, dB1):
    ...
    output = subprocess.run(
        ["ffmpeg", "-i", movie1, "-af",
            "silencedetect=noise={}dB:d=0.3".format(dB1), "-f", "null", "-"],
    ...
    """if str(silence_section_list[0][0]) != "0.0":
        split_file1 = "/var/www/cut/" + movie_name[0] + "_0" + ".mp4"
    ...
    for i in range(len(silence_section_list) - 1):
        split_file = "/var/www/cut/" + \
            movie_name[0] + "_" + str(i + 1) + ".mp4"
        subprocess.run(["ffmpeg", "-ss", str(silence_section_list[i][1]), "-i", movie1, "-t", str(silence_section_list[i + 1]
                       [0] - silence_section_list[i][1]), split_file], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        print(movie_name[0] + "_" + str(i + 1) + ".mp4")


def merge_movie(movie_list1):
    '''無音部分をカットして動画達をマージ'''
    os.chdir("/var/www/merged")
    movie1 = "/var/www/merged/" + movie_list1[0]
    movie_list1 = ["/var/www/cut/" + m for m in movie_list1]
    with open("tmp.txt", "w") as fp:
        lines = [f"file '{line1}'" for line1 in movie_list1]
        fp.write("\n".join(lines))
    subprocess.run(["ffmpeg", "-f", "concat", "-safe", "0", "-i", "tmp.txt",
                   "-c", "copy", movie1], stdout=subprocess.PIPE, stderr=subprocess.PIPE)


if __name__ == "__main__":
    '''メインメソッド'''
    movie_list = get_movie("./movie")
    ...
    movie_list = get_movie("/var/www/cut")
    for movie in movie_list:
